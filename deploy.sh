...
rsync --rsync-path="mkdir -p ${DIR_ON_SERVER} && rsync" -avzh --delete --progress --exclude="certs/concatenated.crt" -e "ssh -i ~/.ssh/skc-server.pem" skc-deck-api certs .env_prod docker-compose.yaml "${USER}@${SERVER}:${DIR_ON_SERVER}/"
